...
class DDPSpawnPlugin(ParallelPlugin):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.num_processes = (
            len(parallel_devices) if parallel_devices is not None else 0
        )
        self.mp_queue = None
    ...
    def init_ddp_connection(
        self, global_rank: Optional[int], world_size: Optional[int]
    ) -> None:
    ...
    def transfer_distrib_spawn_state_on_fit_end(self, results):
        ...
        best_model_path = (
            checkpoint_callback.best_model_path if checkpoint_callback else None
        )
        ...
        if self.global_rank == 0 and self.mp_queue is not None:
            ...
            self.lightning_module.add_to_queue(
                self.mp_queue
    ...
    def __recover_child_process_weights(self, best_path, last_path):
        ...
        if self.lightning_module.trainer.checkpoint_callback:
            self.lightning_module.trainer.checkpoint_callback.best_model_path = (
                best_path
            )
